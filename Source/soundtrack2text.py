...
def process_course_audio(
    soundtrack_folder, transcript_folder, xunfei_appid, xunfei_secret_key
):
    transcripts = {}
    for soundtrack_file in os.listdir(soundtrack_folder):
        soundtrack_file_name = ".".join(soundtrack_file.split(".")[:-1])  # 01
        transcript_file = os.path.join(
            transcript_folder, f"{soundtrack_file_name}.txt"
        )  # Course2Note/OS/Transcript/01.txt

        # Check if transcript file already exists
        if os.path.exists(transcript_file):
            print(f"Transcript file [{transcript_file}]: already exists, skipping")
            with open(transcript_file, "r", encoding="utf-8") as f:
                text = f.read()
        else:
            print(
                f"Transcript file [{transcript_file}]: does not exist, doing API request"
            )
            # Save new transcript
            api = RequestApi(
                appid=xunfei_appid,
                secret_key=xunfei_secret_key,
                upload_file_path=soundtrack_file,
            )
            ...
            with open(transcript_file, "w", encoding="utf-8") as f:
                f.write(text)
        transcripts[soundtrack_file_name] = text
    return transcripts
